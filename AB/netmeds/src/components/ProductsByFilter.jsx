import { Box,Text,Image, Center, Heading,Stack, useColorModeValue, Flex, Button, Input } from '@chakra-ui/react'
...
const ProductsByFilter = () => {
  ...
  return (
    <Box display="flex" >
        {/* <Stack > */}
              <Box >
              <Text m={4} mb={0} fontSize='2xl'>Filters</Text>
              <Input m={4} width={200} type={'text'} placeholder={'search here'} onChange={(e)=>setQ(e.target.value)} />
              <FilterCategories/>
              </Box>
              ...
              <Text pl={8} pt={3} color={'#747789'} fontSize={'large'}>ALL PRODUCTS</Text>
 ...
 function ProductSimple({id,image,salePrice,strikeOfPrice,title,brand,seller,daata}) {
  ...
  return (
    ...
    <img style={{paddingLeft:"14px",paddingTop:"2px"}} src={image} alt=''/>
      <Text color={'black'} fontWeight={500}>{title}</Text>
      <Text>{brand}</Text>
      <Text as={"i"} fontSize="smaller" color={'#717486'}>{seller}</Text><br></br>
      <span style={{color:"#6F7284",fontWeight:"500px"}}>Best Price*  </span>
      <span style={{color:"#EF4281"}}>{salePrice}</span><br></br>
      <span style={{color:"#6F7284"}}>MRP : </span>
      <Text color={'#717486'} as='s'>Rs.{strikeOfPrice}</Text>
